...
if baseline_dir.exists():
    ...
    st.sidebar.write(f"**Baselines**: {len(baselines)}")
st.sidebar.write(f"**Target**: {config.project.target_samples}")
st.sidebar.write(f"**Data**: {config.storage.database_path}")
...
with main_col:
    ...
    # Live feed placeholder - single consolidated view
    ...
    # Display live frame from capture system - SINGLE LIVE FEED ONLY
    if live_frame_path.exists():
        try:
        ...
        # No live frame available - show baseline as fallback with clear indication
        baseline_dir = Path(config.storage.baselines_path)
        if baseline_dir.exists():
            baselines = sorted(baseline_dir.glob("baseline_*.jpg"), reverse=True)
            if baselines:
                frame_placeholder.image(str(baselines[0]), 
                                       caption="📷 Baseline (empty conveyor) - Waiting for capture system",
                                       use_container_width=True)
                status_placeholder.info("The capture system will write live frames when running. Start with: python run.py --mode full")
            else:
                frame_placeholder.info("📷 No camera feed available. Start the capture system to begin.")
                status_placeholder.info("Run: python run.py --mode full")
        else:
            frame_placeholder.info("📷 No camera feed available. Start the capture system to begin.")
            status_placeholder.info("Run: python run.py --mode full")
...
# Recent captures gallery - collapsible to reduce clutter
with st.expander("📸 Recent Captures Gallery", expanded=True):
    recent_captures = db.get_captures(limit=8)
